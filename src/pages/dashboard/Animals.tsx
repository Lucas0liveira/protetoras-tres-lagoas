...
function RegisterAnimalDialog({ open, onClose, onCreated }: {
  ...
  const { register, handleSubmit, watch, setValue, reset, formState: { errors, isSubmitting } } = useForm<AnimalFormValues>({
    ...
    defaultValues: { species: 'canino', sex: 'indefinido', status: 'resgatado' },
  })
  ...
  async function onSubmit(values: AnimalFormValues) {
    const { data: animal, error } = await supabase.from('animals').insert({
      ...
      birth_estimate: values.birth_estimate || null, notes: values.notes || null, status: values.status,
    }).select().single()
...
export default function Animals() {
  ...
  async function fetchAnimals() {
    ...
    else setAnimals(data as Animal[])
    setLoading(false)
  }
            ...
            <tbody>
              {sorted.map(animal => (
                <tr key={animal.id}
                  ...
                  <td className="px-4 py-3 font-medium text-stone-800">{animal.name}</td>
