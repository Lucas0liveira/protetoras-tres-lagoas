...
import { Search, Loader2, ChevronDown, ChevronUp, ChevronsUpDown, Eye, Plus, Pencil, UserCheck, UserX } from 'lucide-react'

import { supabase } from '@/lib/supabase'
import type { Interest, InterestStatusEnum, Colaboradora } from '@/types/database'
...
function ColaboradoraModal({ open, onClose, item, onSaved }: {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const payload = { name: name.trim(), phone: phone || null, email: email || null, notes: notes || null }
...
function ColaboradorasTab() {
  ...
  async function load() {
    ...
    if (error) { toast.error('Erro ao carregar colaboradoras'); setLoading(false); return }
    setItems((data ?? []) as Colaboradora[])
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      <ColaboradoraModal open={modal} onClose={() => setModal(false)} item={editing}
        onSaved={saved => {
...
type Tab = 'formularios' | 'colaboradoras'
  ...
  return (
    <div className="p-4 sm:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-stone-800">Formulários & Equipe</h1>
          <p className="text-stone-400 text-sm mt-1">Formulários de interesse do público e gestão de colaboradoras</p>
        </div>
        {pendingCount > 0 && activeTab === 'formularios' && (
      ...
      <div className="flex gap-1 bg-stone-100 p-1 rounded-lg w-fit mb-6">
        {([
          { key: 'formularios',  label: 'Formulários de Interesse' },
          { key: 'colaboradoras', label: 'Colaboradoras (Equipe)' },
      ...
      {activeTab === 'colaboradoras' && <ColaboradorasTab />}
